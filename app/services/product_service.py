...
from app.utils.redis_cache import delete_cached_data, get_cached_data, set_cached_data
    ...
    def AddProduct(self, request, context):
        ...
        if not name or price <= 0:
            return service_pb2.ProductResponse(
                success=False, message="Invalid product name or price"
            )
...
def create_server():
    auth_interceptor = AuthInterceptor(
        protected_methods=["/app.ProductService/AddProduct"]
    )
    server = grpc.server(
